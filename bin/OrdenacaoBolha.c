...
int partition(Pokemon *lista, int low, int high, int *numComparacoes, int *numMovimentacoes)
{
    ...
    for (int j = low; j < high; j++)
    {
            ...
            (lista[j].generation == pivot.generation && strcmp(lista[j].name, pivot.name) < 0))
        {
            ...
            lista[i] = lista[j];
            lista[j] = temp;
            (*numMovimentacoes) += 3; // Incrementa o contador de movimentações
        }
    }
...
void ordenarPokemons(Pokedex *pokedex, double *tempoExecucao, int *numComparacoes, int *numMovimentacoes)
{
    ...
    *tempoExecucao = ((double)(end - start)) / CLOCKS_PER_SEC * 1000; // Tempo em milissegundos
}
...
int main()
{
    ...
    ordenarPokemons(&pokedex, &tempoExecucao, &numComparacoes, &numMovimentacoes);
