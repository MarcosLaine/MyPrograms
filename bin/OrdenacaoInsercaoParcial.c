...
void insertionSortPartial(Pokemon pokemons[], int n, int k, int *numComparacoes, int *numMovimentacoes)
{
    k = 10;
...
void ordenarPokemons(Pokedex *pokedex, double *tempoExecucao, int *numComparacoes, int *numMovimentacoes, int k)
{
    clock_t start = clock();

    insertionSortPartial(pokedex->listaDePokemons, pokedex->numPokemons, k, numComparacoes, numMovimentacoes);

    clock_t end = clock();
    *tempoExecucao = ((double)(end - start)) / CLOCKS_PER_SEC * 1000;
}
...
int main()
{
    ...
    ordenarPokemons(&pokedex, &tempoExecucao, k, &numComparacoes, &numMovimentacoes);
