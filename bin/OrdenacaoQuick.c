...
void quicksort(Pokemon *lista, int low, int high, int *numComparacoes, int *numMovimentacoes)
{
    if (low < high)
    {
        int pi = partition(lista, low, high, numComparacoes, numMovimentacoes);
        quicksort(lista, low, pi - 1, numComparacoes, numMovimentacoes);
        quicksort(lista, pi + 1, high, numComparacoes, numMovimentacoes);
    }
}

int partition(Pokemon *lista, int low, int high, int *numComparacoes, int *numMovimentacoes)
{
    ...
    for (int j = low; j < high; j++)
    {
            ...
            (lista[j].generation == pivot.generation && strcmp(lista[j].name, pivot.name) < 0))
        {
            ...
            lista[i] = lista[j];
            lista[j] = temp;
            (*numMovimentacoes) += 3; // Incrementa o número de movimentações (2 para a troca + 1 para a variável temp)
        }
    }
...
void ordenarPokemons(Pokedex *pokedex, int *numComparacoes, int *numMovimentacoes)
{
    quicksort(pokedex->listaDePokemons, 0, pokedex->numPokemons - 1, numComparacoes, numMovimentacoes);
}

void salvarLog(Pokedex *pokedex, int numComparacoes)
{
    const char *LOG_FILE_NAME = "803627.txt";
    ...
    time_t t;
    time(&t);
    fprintf(logFile, "Matrícula: 803627\n");
    fprintf(logFile, "Data e Hora: %s", ctime(&t));
...
int main()
{
    ...
    clock_t start = clock();
    ordenarPokemons(&pokedex, &numComparacoes, &numMovimentacoes);
    clock_t end = clock();
    ...
    FILE *logFile = fopen("803627.txt", "w");
    if (logFile)
    {
        fprintf(logFile, "matricula\t%.2lfms\t%ld\t%ld\n", tempoExecucao, numComparacoes, numMovimentacoes);
        fclose(logFile);
    }
    else
    {
