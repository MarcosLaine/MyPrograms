...
void calcularNotas(int valor, int quantidade[]) {
    ...
    if (valor_restante > 0) {
        printf("Não há notas disponíveis para fornecer o valor solicitado.\n");
        printf("Por favor, escolha um valor menor ou reponha o estoque.\n");
        for (int i = 0; i < TOTAL_NOTAS; i++) {
            quantidade[i] = 0;
        }
    }
...
void saque(int valor) {
    ...
    if (valor < 100) {
        printf("Saldo insuficiente! Por favor, reponha mais dinheiro.\n\n");
        return;
    }

    calcularNotas(valor, quantidade);

    bool estoque_insuficiente = false;

    for (int i = 0; i < TOTAL_NOTAS; i++) {
        if (quantidade[i] > 0) {
            printf("%d nota(s) de R$%d\n", quantidade[i], notas[i]);
            estoque[i] -= quantidade[i];
        } else if (estoque[i] < quantidade[i]) {
            estoque_insuficiente = true;
        }
    }

    if (estoque_insuficiente) {
        printf("Estoque insuficiente para fornecer o valor solicitado.\n");
        printf("Por favor, reponha o estoque.\n");
    }
    ...
    while (1) {
        ...
        if (opcao == 'a') {
            ...
            if (senha_correta) {
                ...
                sleep(30);
                tentativas=2; // Bloqueio por 5 minutos (300 segundos)
            }
        } else if (opcao == 'd') {
