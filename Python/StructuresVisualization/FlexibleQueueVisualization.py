...
class FilaVisual:
    def __init__(self, root, tamanho):
        self.root = root
        self.root.title("Visualização da Fila")
        ...
        # Posições dos retângulos da fila
    ...
    def enqueue_com_pausa(self, i, fim):
        if i < fim:
            self.enqueue(self.numeros[i])
            self.root.after(500, self.enqueue_com_pausa, i + 1, fim)  # Pausa de 300 ms entre as inserções

    def executar_passos(self):
        if self.passos == 0:
            # Inserir até completar (com pausa)
            self.enqueue_com_pausa(0, self.tamanho)
        elif self.passos == 1:
            # Remover 2 elementos
            for i in range(2):
                self.dequeue()
